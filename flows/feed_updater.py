...
"""
MediaSource Content Updater

A command-line utility for synchronizing local content databases with the latest video content
from MediaSource affiliate partner offers. This tool automates the process of fetching, processing,
...
Main Functionality:
- Scrapes and parses video content from MediaSource
...
Example:
    >>> from flows.feed_updater import MediaSourceUpdater
    >>> updater = MediaSourceUpdater(img_optimize=True)
...
class MediaSourceUpdater:
    ...
    def cli_arg_updater(self) -> None:
        """
        Process and handle command line arguments for the MediaSource update wizard.
        """

        arg_parser = ArgumentParser(
            description="MediaSource local update wizard arguments"
        )
    ...
    def _print_header(self):
        ...
        self._console.print(
            "Welcome to the MediaSource local update wizard\n",
    ...
    def run(self):
        ...
        with self._console.status(
            "Updating MediaSource artifacts. Please wait...", spinner="bouncingBall"
        ):
            self._flow_start()
        self.print_duration()
