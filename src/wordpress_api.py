...
def map_model_id(wp_posts_f: list[dict]):
    ...
    for elem in wp_posts_f:
        for item in elem['class_list']:
            if re.match("authors", item):
                model = " ".join(item.split('-')[1:]).title()
                if model not in result_dict.keys():
                    # In theory, if the video has at least one model
                    # elem['model'] should not be empty or None.
                    result_dict[model] = elem['authors'][0]
            else:
                continue
    return result_dict
