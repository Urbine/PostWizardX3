...
def get_post_titles_local(
    ...
    for post in wp_posts_f:
        if yoast:
            all_titles.append(
                " ".join(
                    post['yoast_head_json']['title'].split(" ")[
                        :-2]))
        else:
...
def map_tags_posts(wp_posts_f: list[dict], host_name: str = None, idd: str = None) -> dict[str, list[str]]:
...
def get_post_models(wp_posts_f: list[dict]) -> list[str]:
    ...
    models = [[" ".join(cls_lst.split('-')[1:]).title() for cls_lst in elem['class_list']
               if re.match("authors", cls_lst)] for elem in wp_posts_f]
...
def map_wp_class_id(wp_posts_f: list[dict],
                    match_word: str, key_wp: str) -> dict[str, int]:
    """
...
def create_tag_report_excel(wp_posts_f: list[dict],
    ...
    post_id_slug.write_column('C2', unpack_tpl_excel(
        map_postsid_category(wp_posts_f).values()))
...
def upload_thumbnail(
        wp_self: str, param_lst: list[str], file_path: str, payload: dict[str, str | int]) -> int:
...
def update_json_cache(hostname: str, params_dict: dict[str, str], endpoint: str,
                      config_dict: list[dict], wp_filen: str) -> list[dict]:
