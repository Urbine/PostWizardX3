"""
FeedDelta API Integration for content management.
...
from core.config.config_factories import feed_delta_conf_factory
...
class FeedDeltaBaseUrl:
    """
    Builder class for the FeedDelta API Base URL with campaign name.
    """

    def __init__(self, campaign: str = "") -> None:
        workflow_conf = feed_delta_conf_factory()
        campaign_utm = "=".join(workflow_conf.campaign_utm.split("."))
        if campaign:
            self.__campaign = campaign
            self.__feed_delta_base_url = (
                f"https://example-feed.com/content/dump?camp={self.__campaign}&{campaign_utm}"
            )
        else:
            self.__feed_delta_base_url = f"https://example-feed.com/content/dump?{campaign_utm}"

    def __str__(self) -> str:
        return self.__feed_delta_base_url


@dataclass(frozen=True)
class FeedDeltaOptions:
    """
    Builder dataclass for the FeedDelta URL options.
    """
...
@dataclass(frozen=True)
class FeedDeltaFields:
    """
    Builder dataclass for the FeedDelta Dump Fields
    """
...
class FeedDeltaURL:
    """
    Builder class for the first part of FeedDelta URL.
    """
    ...
    def __init__(
        ...
        f_options = FeedDeltaOptions()
        self.base_url = str(FeedDeltaBaseUrl(campaign=campaign))
...
class FeedDeltaFieldStr:
    """
    Builder class for csv/dump columns of the FeedDelta URL (second part).
    """
    ...
    def __init__(
        ...
        fields = FeedDeltaFields().__dict__
...
def feed_delta_parse(
    filename: str, extension: str, dirname: str, sep: str, log_res: bool = False
) -> None:
    """
    Content dump parser for FeedDelta API. It takes, ideally, a ``CSV`` file to
...
def main(*args, **kwargs):
    f_house_url = FeedDeltaURL(*args)
    f_house_fields = FeedDeltaFieldStr(**kwargs)
...
if __name__ == "__main__":
    args_cli = argparse.ArgumentParser(
        description="FeedDelta API implementation for webmaster-seo-tools."
    )
    ...
    args_cli.add_argument(
        ...
        type=str,
        default="single",
        help="Select orientations from options: single, multi (default)",
    )
