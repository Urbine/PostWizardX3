"""
Access MediaSource to get Hosted Videos and links via web automation.
...
from core import helpers, media_source_auth, tasks_conf
from core.config_mgr import TasksConf, MediaSourceAuth
...
def extract_thumbnail(bs4_obj: BeautifulSoup) -> list[str]:
    ...
    poster="https://xyz.com/partner-b/video/video_thumbnail.jpg"></video>``
...
def get_set_source_flow(
    ...
    :param media_source_auth: ``MediaSourceAuth`` with credentials to access the partner site.
    ...
    with webdrv as driver:
        # Go to URL
        print(f"Getting all available Photosets from MediaSource")
        print("Please wait...\n")
        driver.get(task_conf.feed_set_url)
        ...
        username_box.send_keys(media_source_auth.username)
        pass_box.send_keys(media_source_auth.password)
        time.sleep(1)
