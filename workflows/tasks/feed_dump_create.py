"""
MediaSource Video Dump Creator

This module automates the creation of video information dump files from the MediaSource partner platform.
Using web automation via Selenium, it logs into the MediaSource interface, navigates through the site,
and extracts video metadata into structured dump files.

Key features:
- Automated login to MediaSource partner platform
...
import core
from core import media_source_auth, tasks_conf
from core.config_mgr import MediaSourceAuth, TasksConf
...
def get_vid_dump_flow(
    webdrv,
    media_source_info: MediaSourceAuth = media_source_auth(),
    ...
    :param media_source_info: ``MediaSourceAuth`` object with necessary credentials for authentication. (Default)
    ...
    with webdrv as driver:
        ...
        driver.get(task_conf.feed_dump_url)
        driver.implicitly_wait(30)

        username_box = driver.find_element(By.ID, "user")
        pass_box = driver.find_element(By.ID, "password")

        username_box.send_keys(media_source_info.username)
        pass_box.send_keys(media_source_info.password)
        ...
        # Dump format: Dump with | (Select this one on MediaSource)
