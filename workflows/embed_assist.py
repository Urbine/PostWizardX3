...
db_partner_conn, db_partner_cur, db_partner_name = helpers.get_project_db(
    parent=True)
...
# 0. <Description>
# 0. https://example.com/video/353878/sample-video/?campaign_id=1291575419
...
# 0. <tags>
# 0. <model>
# 0. <iframe allowfullscreen="" frameborder="0" height="360" mozallowfullscreen="" msallowfullscreen="" oallowfullscreen="" src="https://partner-six.example.com/embed/353878/?campaign_id=1291575419" webkitallowfullscreen="" width="640"></iframe>
# 0. https://cdn.example.com/contents/videos_screenshots/353000/353878/480x270/
# 0. 1.jpg
# 0. 1.jpg,2.jpg,3.jpg,4.jpg,5.jpg,6.jpg,7.jpg,8.jpg,9.jpg,10.jpg,11.jpg,12.jpg
# 0. https://cdn.example.com/c2/videos/353000/353878/sample_tr.mp4
# 0. sample-video
...
for num, vid in enumerate(videos):
    ...
    if add_post:
        ...
        tag_check = content_select.identify_missing(
        ...
        new_models = content_select.identify_missing(
        ...
        if new_models is None:
            ...
            for author in new_models:
                ...
                pyclip.detect_clipboard()
                pyclip.copy(author)
        # 38 is Sample Category
        category = [38] if partner == "feed_alpha" else None
        payload = content_select.make_payload_simple(
            wp_slug,
            "draft",
            title,
            description,
        ...
        try:
            ...
            content_select.fetch_thumbnail(
            ...
            upload_img = wordpress_api.upload_thumbnail(
                wp_base_url, [
                    "/media"], f"../thumbnails/{wp_slug}.jpg", img_attrs
            )
        ...
        if num < total_elems - 1:
            ...
            if next_post == ("y" or "yes"):
                ...
                try:
                    sync = content_select.hot_file_sync(
                        "wp_posts", "posts_url")
                    ...
                    sync = True
                if sync:
                    not_published_yet = content_select.filter_published(
                        all_vals, wp_posts_f
                    )
                    continue
                else:
