...
def query_modifier(
    sql_query: str,
    bot_config: Union[ContentBotConf, GalleryBotConf, EmbedAssistBotConf],
):
    """
    Write a SQL query to the configuration file.

    :param sql_query: ``str`` -> The SQL query to set.
    :param bot_config: ``Union[ContentBotConf, GalleryBotConf, EmbedAssistBotConf]`` -> The bot configuration object.
    :return: ``bool`` -> ``True`` if the write operation was successful, ``False`` otherwise.
    """
    if isinstance(bot_config, ContentBotConf):
        return ContentBotConfig.write_db_query(sql_query)
    elif isinstance(bot_config, GalleryBotConf):
        return GalleryBotConfig.write_db_query(sql_query)
