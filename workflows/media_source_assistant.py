...
def video_upload_pilot(
    ...
    for num, vid in enumerate(not_published_yet):
        ...
        if add_post:
            slugs: list[str] = [
                f"{fields[8]}-video",
                workflows.make_slug(partner, models, title, "video"),
                workflows.make_slug(partner, models, title, "video", reverse=True),
                workflows.make_slug(partner, models, title, "video", partner_out=True),
            ]

            wp_slug = workflows.slug_getter(console, slugs)
            ...
            partner_tag: str = workflows.clean_partner_tag(partner.lower())
            ...
            categ_ids = workflows.pick_classifier(
                console, wp_posts_f, title, description, tags
            )
