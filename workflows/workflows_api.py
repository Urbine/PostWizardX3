...
- Content Selection (media_source_assistant.py)
...
class EmbedsMultiSchema(Generic[T]):
    ...
    class SchemaRegEx:
        ...
        pat_authors: Pattern[str] = re.compile(r"authors?", re.IGNORECASE)
        ...
        pat_content_format: Pattern[str] = re.compile(r"orientation", re.IGNORECASE)
    ...
    def get_authors(self) -> Optional[str | int]:
        """Models getter/reader from the matched fields in the data structure in an instance of
            ``EmbedsMultiSchema``

        :return: ``Optional[str | int]`` - Data from ``self.__data`` located in the ``authors`` index.
        """
        re_pattern: Pattern[str] = EmbedsMultiSchema.SchemaRegEx.pat_authors
        return self.__safe_retrieve(re_pattern)
    ...
    def get_content_format(self) -> Optional[str | int]:
        """ContentFormat getter/reader from the matched fields in the data structure in an instance of
            ``EmbedsMultiSchema``

        :return: ``Optional[str | int]`` - Data from ``self.__data`` located in the ``orientation`` index.
        """
        re_pattern: Pattern[str] = EmbedsMultiSchema.SchemaRegEx.pat_content_format
        return self.__safe_retrieve(re_pattern)
...
def get_tag_ids(wp_posts_f: list[dict], tag_lst: list[str], preset: str) -> list[int]:
    ...
    WordPress handles tag string input the same way; for example, if I write the tag 'featured' into the tags
    textarea in the browser, it maps it to the previously recorded 'Featured' tag, so theoretically and,
    as a matter of fact,'featured' and 'Featured' map to the same tag ID within the WordPress site.
...
def make_payload_simple(
    ...
    if categs:
        ...
        payload_post["authors"] = model_int_lst
...
def hot_file_sync(
    ...
    :param bot_config: ``ContentBotConf`` or ``GalleryBotConf`` or ``EmbedAssistBotConf`` with configuration information.
...
def telegram_send_message(
    ...
    :param msg_text: ``str``
    :param post_url: ``str``
    :param bot_config: ``ContentBotConf`` | ``EmbedAssistBotConf`` | ``GalleryBotConf``
    :return: ``int``
    """
...
def wp_publish_checker(post_slug: str, cs_conf) -> Optional[bool]:
    ...
    :param cs_conf: ``ContentBotConf`` | ``EmbedAssistBotConf`` | ``GalleryBotConf`` - Config objects
    :return: ``None`` | ``True``
    """
    retries = 0
    retry_offset = 5
...
def social_sharing_controller(
    ...
    :param cs_config: ``ContentBotConf`` | ``GalleryBotConf`` | ``EmbedAssistBotConf`` workflow config object
    :return: ``None``
    """
...
def model_checker(
    ...
    if new_models is None:
        ...
        for author in new_models:
            ...
            console.print(
                "Paste it into the models field as soon as possible...\n",
                style=ConsoleStyle.TEXT_STYLE_ATTENTION.value,
            )
            logging.warning(f"Missing model: {author}")
            pyclip.detect_clipboard()
            pyclip.copy(author)
...
def fetch_zip(
    ...
    gecko: bool = False,
    headless: bool = False,
    media_source_auth: MediaSourceAuth = media_source_auth(),
) -> None:
    ...
    **For this reason, refrain from using headless mode with this module.**
    ...
    :param media_source_auth: ``MediaSourceAuth`` object with authentication information to access MediaSource.
    :return: ``None``
    """
    ...
    username: str = media_source_auth.username
    password: str = media_source_auth.password
...
def make_gallery_payload(
    ...
    :param gallery_sel_conf: ``GalleryBotConf`` - Bot configuration object
    :return: ``dict[str, str]``
    """
...
def filter_relevant(
    ...
    models_set: set[str] = set(
        wordpress_api.map_wp_class_id(wp_posts_f, "authors", "authors").keys()
    )
    not_published_yet = []
