...
def media_source_remove_credentials(toggle_notification=True) -> None:
    """
    This function removes the MediaSource affiliate program credentials from the secret vault.

    :return: ``None``
    """
    return info_delete_secret(
        SECRET_HANDLER.delete_secret(SecretType.MEDIA_SOURCE_PASSWORD),
        toggle_notification=toggle_notification,
    )


def media_source_update_credentials(*args) -> None:
    """
    This function updates and stores the MediaSource affiliate program credentials in the secret vault.

    :param args: Additional arguments to pass to the secret handler.
    :return: ``None``
    """
    secrets = SECRET_HANDLER.get_secret(SecretType.MEDIA_SOURCE_PASSWORD)
    if secrets:
        media_source_remove_credentials(toggle_notification=False)
    return store_secrets(SecretType.MEDIA_SOURCE_PASSWORD, *args)
