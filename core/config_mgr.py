...
class MediaSourceAuth:
    """
    Immutable dataclass with the MediaSource secret structure.
    """
...
def media_source_auth() -> MediaSourceAuth:
    """Factory function for dataclass ``MediaSourceAuth``

    :return: ``MediaSourceAuth``
    """
    try:
        return MediaSourceAuth(
            username=CLIENT_INFO_INI["MediaSource"]["username"],
            password=CLIENT_INFO_INI["MediaSource"]["password"],
        )
    except KeyError:
        raise ClientInfoSecretsNotFound(CONFIG_PATH)
...
def update_media_source_conf() -> UpdateMediaSource:
    """Factory function for dataclass ``UpdateMediaSource``
...
def tasks_conf() -> TasksConf:
    ...
    """
    return TasksConf(
        feed_dump_url=TASKS_CONFIG_INI["dump_create_config"]["feed_dump_url"],
        feed_set_url=TASKS_CONFIG_INI["dump_create_config"]["feed_set_url"],
    )
