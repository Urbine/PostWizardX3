...
.. attention::
   ...
   On the other hand, if you really intend to run this program, contact MediaSource or go to their image
...
At runtime, the programs reading configuration files make an assumption about expected conditions, for example:
**If you added an x values, you need to provide x data sources.**

In this context, an ``InvalidInput`` exception may be raised by any of the content bots if you provided
more values than data sources in their configuration file. A case in point that is a common driver for
users of this application is adding x number of partners and omit one of the corresponding databases.

.. attention::
   Make sure to have sufficient data sources that correspond with the values you add in ``workflows_config.ini``,
   specifically the ``partners`` entry and their databases generated by service integrations in the project.

   You can resolve this issue by removing extra partners, so that bots can match available
   databases with their corresponding offers.
...
.. note::
   ...
   than non-headless mode. For ``workflows.update_media_source_chain``, I applied a configuration option to disable
