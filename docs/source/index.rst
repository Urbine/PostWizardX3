...
content upload for websites built on top of WordPress.
...
5. Add your additional credentials in the configuration ``client_info.ini``.
   ...
   a. MediaSource Affiliate program

   .. code:: console

      $ python3 -m workflows.update_media_source_chain --hint <partner_hint> --gecko --headless
   ...
   b. FeedBeta Feeds (PartnerEight, PartnerNine)
   ...
   c. PartnerSeven API
      ...
      For PartnerSeven, the default day count is 30 days. However, you can experiment with any value that works for the task at hand.
...
1. ``content_select`` is in charge of reading the databases produced by tasks associated with the
   MediaSource Affiliate Program and uploading video information to WordPress.

   .. code:: console

      $ python3 -m workflows.content_select

2. ``gallery_select`` is another bot that deals with information from MediaSource.
   ...
   .. warning::
      ...
      **This is one of those cases.** The ``--gecko`` flag can be omitted altogether and default to
      ...
      until current downloads finish before closing the browser instance; fixes have been applied here to correct that behaviour.
      ...
      downloaded and the browser instance could then be closed either automatically or explicitly.
...
3. ``embed_assist`` interacts with other content APIs that rely on embed codes to display content
   on WordPress. It reads, processes, and uploads the information from the PartnerSeven and FeedBeta feeds
...
4. ``update_mchash_chain`` controls the tasks developed in the ``tasks`` package
   ...
   It is useful when MediaSource releases updates as this bot will update the databases and rebuild them.
   ...
   .. note::
      ``--headless`` mode is fully supported in ``workflows.update_media_source_chain`` and use of ``--gecko``
