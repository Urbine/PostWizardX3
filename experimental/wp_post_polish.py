...
with web_driver as driver:
    ...
    math_challenge_label = driver.find_element(
        By.XPATH, '//*[@id="loginform"]/div[2]/label'
    )
    ...
    # ==== Yoast SEO Fields ====
    focus_keyphrase = driver.find_element(
        By.XPATH, "//input[@id='focus-keyword-input-metabox']"
    )
    focus_keyphrase.click()
    focus_keyphrase.send_keys("Python")

    # TODO: Get the progress <div> element to measure SEO Meta description and
    # slug length
    post_slug = driver.find_element(
        By.XPATH, "//input[@id='yoast-google-preview-slug-metabox']"
    )
    post_slug.click()
    post_slug.clear()
