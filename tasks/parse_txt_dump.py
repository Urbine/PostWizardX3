...
if __name__ == "__main__":
    ...
    db_name_suggest = [
        f"partner_a-vids-{datetime.date.today()}.db",
        f"partner3-vids_{datetime.date.today()}.db",
        f"partner2-vids-{datetime.date.today()}.db",
    ]

    db_name = helpers.filename_creation_helper(db_name_suggest, extension="db")

    db_conn = sqlite3.connect(
        f"{helpers.is_parent_dir_required(parent=True)}{db_name}")
    ...
    try:
        ...
        raise IndexError(
            f"There are {len(txt_files)} .txt files in {os.path.dirname(os.getcwd())}\nTry again!"
        )
