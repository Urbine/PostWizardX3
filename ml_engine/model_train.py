...
def get_training_set(
        training_set: list[tuple[str, str, str]]) -> list[tuple[str, str, str]]:
...
def wp_get_training_data(
    ...
    raw_descriptions = wordpress_api.get_post_descriptions(
        import_wp_cache, yoast=True)
...
stop_words_english.update(['.', ',', '"', "'", '?', '¿', '¡', '!', ':',
                           ';', '(', ')', '[', ']', '{', '}', '&amp;',
                           '&period', 'example&period;com&comma', 'example', '&', '#'])

# Obtain all the vocabulary for preliminary preparation.
vocabulary_titles = set(chain(
    *[word_tokenize(t_data[0].lower()) for t_data in title_desc_training_data]))
vocabulary_descriptions = set(chain(
    *[word_tokenize(t_data[1].lower()) for t_data in title_desc_training_data]))
...
if __name__ == '__main__':
    ...
    NaiveBClassifier_descriptions = NaiveBayesClassifier.train(
